...
// Browsers will now use the /img/axalogo.png defined in the HTML headers
...
app.get('/invoices/:id/receipt', requireAuth, async (req, res) => {
  try {
    ...
    res.send('Error memuat kuitansi.');
  }
});
...
app.get('/invoice/:publicId/receipt', async (req, res) => {
  try {
    ...
    res.status(500).send('Terjadi kesalahan muat kuitansi public.');
  }
});
